...
class Language(db.Model):
    ...
    native_name: Mapped[str] = mapped_column(db.String(50), nullable=False)
    script: Mapped[str] = mapped_column(db.String(30), nullable=False)
    region: Mapped[Optional[str]] = mapped_column(db.String(100))
    speakers_millions: Mapped[Optional[float]] = mapped_column(db.Float)
    official_in: Mapped[Optional[str]] = mapped_column(db.String(200))
    sample_hello: Mapped[Optional[str]] = mapped_column(db.String(100))
    flag_emoji: Mapped[Optional[str]] = mapped_column(db.String(5), default="🇮🇳")
...
class TranslationLog(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    source_text: Mapped[str] = mapped_column(db.Text, nullable=False)
    translated_text: Mapped[str] = mapped_column(db.Text, nullable=False)
    target_lang_code: Mapped[str] = mapped_column(db.String(10), nullable=False, index=True)
    target_lang_name: Mapped[str] = mapped_column(db.String(50), nullable=False)
    created_at: Mapped[datetime] = mapped_column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    def __repr__(self):
        return f"<TranslationLog {self.target_lang_code}: {self.source_text[:20]}...>"
